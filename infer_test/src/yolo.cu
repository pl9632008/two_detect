...
InstanceSegmentMap::InstanceSegmentMap(int width, int height,int left,int top) {
  this->width = width;
  this->height = height;
  this->left = left;
  this->top = top;
      ...
      for (int i = 0; i < count; ++i) {
        ...
        if (keepflag == 1) {
          ...
          if (has_segment_) {
            ...
            int seg_left=scale_to_predict_x*left;
            int seg_top=scale_to_predict_y*top;
            if (mask_out_width > 0 && mask_out_height > 0) {
              ...
              result_object_box.seg =
                  make_shared<InstanceSegmentMap>(mask_out_width, mask_out_height,seg_left,seg_top);
